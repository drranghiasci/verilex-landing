...
export default async function handler(
  ...
  if (typeof updates.title === 'string') nextUpdates.title = updates.title.trim();
  if (typeof updates.description === 'string' || updates.description === null) {
    nextUpdates.description = updates.description;
  }
  if (typeof updates.due_date === 'string') nextUpdates.due_date = updates.due_date.trim();
  if (typeof updates.status === 'string') nextUpdates.status = updates.status.trim();
  if (nextUpdates.status === 'done') {
    nextUpdates.completed_at = new Date().toISOString();
  }
  if (nextUpdates.status === 'open') {
